...
export class AudioStreamProcessor {
  ...
  async startProcessing(stream: MediaStream): Promise<void> {
    try {
      ...
      this.processor.onaudioprocess = (event) => {
        ...
        if (contextSampleRate !== targetSampleRate) {
          ...
          for (let i = 0; i < newLength; i++) {
            ...
            if (srcIndexInt + 1 < audioData.length) {
              resampledData[i] = audioData[srcIndexInt] * (1 - fraction) +
                                audioData[srcIndexInt + 1] * fraction;
            } else {
              resampledData[i] = audioData[srcIndexInt];
            }
          }
        ...
        if (this.bufferQueue.length >= 2) { // Process every 2 buffers
          this.processBatch();
        }
      };
  ...
  private processBatch(): void {
    try {
      ...
      // Send raw PCM Float32 data as base64
